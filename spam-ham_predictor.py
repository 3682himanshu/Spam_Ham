...
def spam_ham_predictor(corpus):
	...
	train_M, test_M, train_L, test_L = train_test_split(messages, labels,random_state=1)
	print("Size of Data = ",len(messages),"Size of Train Data = ",len(train_M),"Size of Test Data = ",len(test_M),sep=' ')
	...
	for lbl,msg in zip(test_L, test_M):
		...
		if prediction[0] != lbl:
			err+=1
	print('Acuraccy = ',(tot-err),'/',tot," = ",(float(tot-err)/float(tot)*100))

spam_ham_predictor('F:/Project/SMSSpamCollection')